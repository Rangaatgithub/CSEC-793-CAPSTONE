...
# Function to preprocess the transcription
def preprocess_transcription(transcription):
    ...
    # Capitalize the transcription if it's in lowercase
...
directory = Path('C:/AudioCAPTCHASamples/BDAFINALAUDIO')

# Load reference transcriptions
reference_transcriptions = {}
for ref_file in Path('C:/AudioCAPTCHASamples/RT').glob('*.txt'):
...
audio_files = sorted(directory.glob('BDA10.wav'), key=lambda x: int(x.stem[3:]))
...
workbook.save("BDATranscriptions.xlsx")
