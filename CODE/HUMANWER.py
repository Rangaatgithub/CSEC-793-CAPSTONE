...
for row in range(2, total_rows + 1):  # Start from row 2 to skip header
    ...
    # Convert human transcriptions to uppercase
    human_transcribed_upper = human_transcribed.upper()
    
    # Calculate WER
    wer_percent = wer(ground_truth, human_transcribed_upper)
    
    # Write data to the new sheet
    new_sheet.cell(row=row, column=1).value = human_transcribed_upper
    new_sheet.cell(row=row, column=2).value = ground_truth
    new_sheet.cell(row=row, column=3).value = wer_percent

# Save the new workbook for BDA
new_workbook.save("C:/AudioCAPTCHASamples/HWER.xlsx")
